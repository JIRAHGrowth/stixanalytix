...
const EVENT_TYPES = ["Corner", "Cross", "Shot", "1v1"];
const GK_ACTIONS_CROSS = ["Claim", "Punched", "Missed/Misjudged"];
const GK_ACTIONS_SHOT = ["Claim", "Parry", "Dive", "Block", "Tip", "Punched", "Missed/Misjudged"];
...
function Counter({ label, value, onChange, min = 0, compact, color }) {
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: compact ? "4px 0" : "6px 0" }}>
      <span style={{ fontSize: compact ? 12 : 13, color: t.text, fontWeight: 500 }}>{label}</span>
      <div style={{ display: "flex", alignItems: "center", gap: 0 }}>
        ...
        <div style={{
          ...
          fontSize: 16, fontWeight: 700, color: color || t.bright,
        }}>{value}</div>
...
function Chip({ label, selected, onClick, small, color }) {
  ...
  return (
    <button onClick={onClick} style={{
      padding: small ? "6px 8px" : "10px 10px", borderRadius: 8,
      border: `1px solid ${selected ? c : t.border}`,
      background: selected ? c + "25" : t.bg,
      color: selected ? c : t.dim,
      fontSize: small ? 11 : 12, fontWeight: selected ? 700 : 500, cursor: "pointer",
      transition: "all 0.12s", textAlign: "center", lineHeight: 1.2, fontFamily: font,
    }}>{label}</button>
  );
}

function Zone({ title, icon, open, onToggle, children, badge, accentColor }) {
  const ac = accentColor || t.accent;
  return (
    <div style={{
      background: t.card, border: `1px solid ${open ? ac + "44" : t.border}`,
      ...
      <button onClick={onToggle} style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          {badge != null && (
            <span style={{ fontSize: 10, fontWeight: 700, color: ac, background: ac + "22", padding: "2px 7px", borderRadius: 5 }}>{badge}</span>
          )}
        </div>
        <span style={{ fontSize: 14, color: t.dim, transform: open ? "rotate(180deg)" : "rotate(0deg)", transition: "transform 0.2s" }}>▼</span>
      </button>
...
// ═══ GOAL ZONE MAP ═════════════════════════════════════════════════════════
function GoalZoneMap({ selected, onSelect }) {
  const w = 300, h = 180;
  const cols = 3, rows = 3;
...
function HalfSummary({ half, events, halves, goalsFor, onClose, clubName, opponent }) {
  ...
  const shots = hEvents.filter(e => e.type === "Shot" || e.type === "1v1");
  const saves = hEvents.filter(e => e.gkAction && e.gkAction !== "Missed/Misjudged" && !e.isGoal && (e.type === "Shot" || e.type === "1v1"));
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            {goals.map((gl, i) => (
              <div key={i} style={{ fontSize: 11, color: t.text, padding: "4px 0", borderBottom: `1px solid ${t.border}11` }}>
                {gl.method === "Own Goal" ? "🟣 OG" : "🚨"} {gl.type} • {SHOT_ORIGINS.find(o => o.id === gl.origin)?.label || gl.origin}
                {gl.goalZone ? ` • ${gl.goalZone}` : ""}{gl.rank ? ` • ${gl.rank}` : ""}
              </div>
            ))}
          </div>
        )}
        <div style={{ fontSize: 12, color: t.text, lineHeight: 1.8, marginBottom: 16 }}>
          <div>Saves: {saves.length} ({saves.map(s => s.gkAction).filter((v, i, a) => a.indexOf(v) === i).join(", ") || "—"})</div>
          <div>Crosses/Corners: {crosses.length} — Claimed: {claims.length}</div>
          <div>Rebounds: {halves[half]?.rbControlled || 0} controlled, {halves[half]?.rbDangerous || 0} dangerous</div>
        </div>
        {halves[half]?.note && (
          <div style={{ fontSize: 12, color: t.gold, fontStyle: "italic", padding: "8px 10px", background: t.gold + "11", borderRadius: 8, marginBottom: 12 }}>
            &ldquo;{halves[half].note}&rdquo;
          </div>
        )}
        <button onClick={onClose} style={{ width: "100%", padding: 12, borderRadius: 10, border: "none", background: t.accent, color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: font }}>Back to Match</button>
      </div>
    </div>
  );
}
...
export default function PitchsidePage() {
  ...
  // ─── Setup state (SIMPLIFIED for Holland MVP)
  ...
  // ─── UI toggles
  const [openZones, setOpenZones] = useState({ distribution: false, sweeper: false, rebounds: false, notes: true });
  const [showHalfSummary, setShowHalfSummary] = useState(null);
  const [showSetup, setShowSetup] = useState(false);
  const [saveError, setSaveError] = useState("");

  // ─── Load keepers from Supabase
  useEffect(() => {
  ...
  const toggleZone = (z) => setOpenZones(prev => ({ ...prev, [z]: !prev[z] }));
  ...
  // Derived stats
  const shotEvents = events.filter(e => e.type === "Shot" || e.type === "1v1");
  const totalSOT = shotEvents.filter(e => !e.offTarget).length;
  ...
  const isSaveAction = (action) => action && action !== "Missed/Misjudged";
  ...
  const needsGoalZone = () => {
    ...
    if ((evtType === "Shot" || evtType === "1v1") && isSaveAction(evtAction)) return true;
    return false;
  };
  const needsMethod = () => (evtType === "Shot" || evtType === "1v1") || evtIsGoal;
  const needsPosRank = () => evtIsGoal;
  ...
  const logEvent = () => {
    const evt = {
      ...
      isGoal: evtIsGoal, offTarget: false, half,
  ...
  const canLog = () => {
    ...
    if (evtType !== "Corner" && !evtOrigin) return false;
  ...
  const saveToDatabase = async () => {
    ...
    try {
      // Aggregate distribution across halves
      const sumHE = (key) => ["H1", "H2", "ET"].reduce((s, hf) => s + (halfExtras[hf]?.[key] || 0), 0);
      const sumH = (key) => ["H1", "H2", "ET"].reduce((s, hf) => s + (halves[hf]?.[key] || 0), 0);

      // Aggregate save types from events
      const saveEvents = events.filter(e => e.gkAction && e.gkAction !== "Missed/Misjudged" && !e.isGoal && (e.type === "Shot" || e.type === "1v1"));
      const countAction = (a) => saveEvents.filter(e => e.gkAction === a).length;

      // Aggregate cross stats
      ...
      const { data: matchData, error: matchError } = await supabase
        ...
        .insert({
          ...
          saves_catch: countAction("Claim"),
          saves_parry: countAction("Parry"),
          saves_dive: countAction("Dive"),
          saves_block: countAction("Block"),
          saves_tip: countAction("Tip"),
          saves_punch: countAction("Punched"),
          crosses_claimed: crossClaimed,
          crosses_punched: crossPunched,
          crosses_missed: crossMissed,
          crosses_total: crossEvents.length,
          dist_gk_short_att: sumHE("dGkShort"),
          dist_gk_short_suc: sumHE("dGkShortOk"),
          dist_gk_long_att: sumHE("dGkLong"),
          dist_gk_long_suc: sumHE("dGkLongOk"),
          dist_throws_att: sumHE("dThrow"),
          dist_throws_suc: sumHE("dThrowOk"),
          dist_passes_att: sumHE("dPass"),
          dist_passes_suc: sumHE("dPassOk"),
          dist_under_pressure_att: sumHE("dPressure"),
          dist_under_pressure_suc: sumHE("dPressureOk"),
          one_v_one_faced: oneV1.length,
          one_v_one_won: oneV1Won,
          errors_leading_to_goal: errorsToGoal,
          sweeper_clearances: sumHE("swClear"),
          sweeper_interceptions: sumHE("swIntercept"),
          sweeper_tackles: sumHE("swTackle"),
          rebounds_controlled: sumH("rbControlled"),
          rebounds_dangerous: sumH("rbDangerous"),
          notes: allNotes || null,
          was_subbed: wasSub,
      ...
      // 2. Insert goals conceded
      if (goalEvents.length > 0) {
        const goalRows = goalEvents.map(g => ({
          match_id: matchData.id,
          coach_id: user.id,
          goal_zone: g.goalZone || null,
          shot_origin: g.origin || null,
          goal_source: g.type === "Corner" ? "Corner" : g.type === "Cross" ? "Open Play" : "Open Play",
          goal_rank: g.rank || null,
          shot_type: g.method || null,
      ...
      const attrKeys = {
        ...
        "Set Piece Org.": "set_piece_org",
        "Footwork & Agility": "footwork_agility",
        "Reaction Speed": "reaction_speed",
        "Communication": "communication",
        "Command of Box": "command_of_box",
        "Composure": "composure",
      ...
      Object.entries(attrKeys).forEach(([label, col]) => {
        attrRow[col] = attrs[label] || null;
      });
      ...
      <div style={{ minHeight: "100vh", background: t.bg, fontFamily: font, color: t.text }}>
        <div style={{ maxWidth: 520, margin: "0 auto", padding: "32px 16px" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28 }}>
            <Link href="/dashboard" style={{ color: t.dim, textDecoration: "none", fontSize: 13 }}>← Dashboard</Link>
          ...
          <div style={{ textAlign: "center", marginBottom: 28 }}>
            ...
            <p style={{ fontSize: 13, color: t.dim, marginTop: 4 }}>
              {club?.name || "Your Club"} — set up, then start tracking.
            </p>
          </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 6 }}>
                {keepers.map(k => (
                  <button key={k.id} onClick={() => setSelectedKeeperId(k.id)} style={{
                    ...
                    <div>
                      <div style={{ fontSize: 14, fontWeight: 600, color: selectedKeeperId === k.id ? t.bright : t.text }}>{k.name}</div>
                      <div style={{ fontSize: 11, color: t.dim }}>{k.role || "—"}</div>
                    </div>
                    {selectedKeeperId === k.id && (
                      <span style={{ marginLeft: "auto", color: club?.primary_color || t.accent, fontWeight: 700 }}>✓</span>
                    )}
                  </button>
                ))}
              </div>
          ...
          <button onClick={() => { if (ready) setPhase("match"); }} disabled={!ready} style={{
            ...
            boxShadow: ready ? `0 4px 20px ${t.accent}44` : "none", fontFamily: font,
        ...
        <div style={{ maxWidth: 520, margin: "0 auto", padding: "24px 16px" }}>
          <div style={{ textAlign: "center", marginBottom: 20 }}>
            ...
            <p style={{ fontSize: 13, color: t.dim }}>
              {subReason ? `${activeKeeper} — ${subReason}` : `Rate ${activeKeeper}'s performance`}
            </p>
          </div>
          ...
          {saveError && (
            <div style={{ background: "#ef444415", border: `1px solid ${t.red}30`, borderRadius: 8, padding: "10px 14px", marginBottom: 14, color: t.red, fontSize: 13 }}>
              {saveError}
            </div>
          )}
    ...
    return (
      <div style={{ minHeight: "100vh", background: t.bg, fontFamily: font, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ textAlign: "center" }}>
          ...
          <div style={{ color: t.dim, fontSize: 13, marginTop: 8 }}>This takes a few seconds.</div>
        </div>
      </div>
    );
  }
  ...
  if (phase === "saved") {
    ...
    const crossEvents = events.filter(e => e.type === "Cross" || e.type === "Corner");
    const claims = crossEvents.filter(e => e.gkAction === "Claim").length;
    const claimRate = crossEvents.length > 0 ? ((claims / crossEvents.length) * 100).toFixed(0) + "%" : "—";
    ...
    return (
      <div style={{ minHeight: "100vh", background: t.bg, fontFamily: font, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24 }}>
        ...
        <p style={{ color: t.dim, fontSize: 13, textAlign: "center" }}>Stored in the cloud. Viewable on your dashboard.</p>
        ...
        <div style={{ background: t.card, borderRadius: 12, padding: 16, marginTop: 20, width: "100%", maxWidth: 340, border: `1px solid ${t.border}` }}>
          {[
            ...
            ["Cross Claim", `${claims}/${crossEvents.length} — ${claimRate}`, null],
        ...
        <div style={{ display: "flex", gap: 12, marginTop: 24 }}>
          <button onClick={() => {
            ...
            setSessionType("match"); setHomeAway(null);
            setMatchDate(new Date().toISOString().split("T")[0]);
          }} style={{
            padding: "13px 28px", borderRadius: 10, background: club?.primary_color || t.accent,
            color: "#fff", border: "none", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: font,
          }}>New Session</button>
          <Link href="/dashboard" style={{
            padding: "13px 28px", borderRadius: 10, border: `1px solid ${t.border}`,
      ...
      <div style={{ position: "sticky", top: 0, zIndex: 100, background: t.card, borderBottom: `1px solid ${t.border}` }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 14px" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <button onClick={() => setShowSetup(true)} style={{ background: t.bg, border: `1px solid ${t.border}`, borderRadius: 6, padding: "4px 8px", fontSize: 11, color: t.dim, cursor: "pointer" }}>⚙</button>
          </div>
        </div>
        ...
        {sessionType === "match" && (
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 12, padding: "8px 14px", borderTop: `1px solid ${t.border}22` }}>
            ...
            <div style={{ display: "flex", gap: 4, marginLeft: 6 }}>
              <button onClick={() => setGoalsFor(goalsFor + 1)} style={{ padding: "6px 10px", borderRadius: 8, border: `1px solid ${t.green}44`, background: t.green + "15", color: t.green, fontSize: 12, fontWeight: 700, cursor: "pointer" }}>⚽+</button>
              {goalsFor > 0 && <button onClick={() => setGoalsFor(goalsFor - 1)} style={{ padding: "6px 8px", borderRadius: 8, border: `1px solid ${t.border}`, background: "transparent", color: t.dim, fontSize: 12, cursor: "pointer" }}>−</button>}
            </div>
          </div>
        )}
        ...
        <div style={{ display: "flex", gap: 3, padding: "6px 14px 10px" }}>
          {HALVES.map(hf => {
            ...
            return (
              <button key={hf} onClick={() => { setHalf(hf); resetEvt(); }} style={{
                ...
                fontSize: 12, fontWeight: half === hf ? 700 : 500, position: "relative", fontFamily: font,
              }}>
                {hf}
          ...
          <button onClick={() => { setShowPSO(!showPSO); resetEvt(); }} style={{
            ...
            fontSize: 12, fontWeight: showPSO ? 700 : 500, fontFamily: font,
          }}>PKs</button>
          {!showPSO && (
            <button onClick={() => setShowHalfSummary(half)} style={{
              padding: "8px 10px", borderRadius: 8, border: `1px solid ${t.gold}44`,
              cursor: "pointer", background: t.gold + "11", color: t.gold, fontSize: 11, fontWeight: 700,
            }}>📋</button>
          )}
        </div>
      </div>
        ...
        {showPSO ? (
          <div style={{ background: t.card, border: `1px solid ${t.gold}44`, borderRadius: 12, padding: 16, marginBottom: 10 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
              ...
              <span style={{ fontSize: 15, fontWeight: 700, color: t.bright }}>Penalty Shootout</span>
            </div>
          ...
          <>
            {/* ═══ EVENT LOGGING FLOW ═══ */}
            <div style={{ background: t.card, border: `1px solid ${(club?.primary_color || t.accent)}44`, borderRadius: 12, padding: "12px 14px", marginBottom: 8 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                <span style={{ fontSize: 14 }}>🧤</span>
                <span style={{ fontSize: 13, fontWeight: 700, color: t.bright }}>Log Event</span>
                {evtType && (
                  <button onClick={resetEvt} style={{ marginLeft: "auto", background: t.red + "22", border: "none", color: t.red, borderRadius: 6, padding: "4px 8px", fontSize: 10, cursor: "pointer", fontWeight: 600, fontFamily: font }}>✕ Cancel</button>
                )}
              </div>

              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 4, marginBottom: 8 }}>
                {EVENT_TYPES.map(et => (
                  <Chip key={et} label={et} selected={evtType === et} onClick={() => { resetEvt(); setEvtType(et); }} small
                    color={evtType === et ? (club?.primary_color || t.accent) : undefined} />
                ))}
              </div>
                  ...
                  {evtType === "Corner" ? (
                    <>
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
                        <Chip label="Left" selected={evtOrigin === "cornerL"} onClick={() => setEvtOrigin("cornerL")} small />
                        <Chip label="Right" selected={evtOrigin === "cornerR"} onClick={() => setEvtOrigin("cornerR")} small />
                      </div>
                    </>
                  ) : (
                    <>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 4 }}>
                    {getAvailableActions().map(a => (
                      <Chip key={a} label={a} selected={evtAction === a} small
                        color={a === "Missed/Misjudged" ? t.red : (club?.primary_color || t.accent)}
                        onClick={() => {
                          setEvtAction(a);
                          if (a === "Missed/Misjudged" && (evtType === "Shot" || evtType === "1v1")) {
                            setEvtIsGoal(true);
                          }
                        }}
                      />
              ...
              {/* Goal? */}
              {evtAction === "Missed/Misjudged" && !evtIsGoal && (
                <div style={{ marginBottom: 8 }}>
                  <div style={{ fontSize: 10, color: t.dim, fontWeight: 600, textTransform: "uppercase", marginBottom: 6 }}>Did it result in a goal?</div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
                    <Chip label="Yes — Goal" selected={evtIsGoal} onClick={() => setEvtIsGoal(true)} small color={t.red} />
                    <Chip label="No — Cleared" selected={!evtIsGoal} onClick={() => setEvtIsGoal(false)} small color={t.green} />
                  </div>
                </div>
              )}
              ...
              {evtAction && needsGoalZone() && !evtGoalZone && (
                <div style={{ marginBottom: 8 }}>
                  ...
                  <GoalZoneMap selected={evtGoalZone} onSelect={setEvtGoalZone} />
                </div>
              )}

              {/* Shot method */}
              {evtAction && (evtGoalZone || !needsGoalZone()) && needsMethod() && !evtMethod && (
                <div style={{ marginBottom: 8 }}>
                  ...
                  <div>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                      {GK_POSITIONING.map(p => (
                        <Chip key={p} label={p} selected={evtPosition === p} onClick={() => setEvtPosition(p)} small color={p === "Set" ? t.green : t.orange} />
                      ))}
                    </div>
                  </div>
                  <div>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                      {GOAL_RANKS.map(r => (
                        <Chip key={r} label={r} selected={evtRank === r} onClick={() => setEvtRank(r)} small
              ...
              {canLog() && (
                <button onClick={logEvent} style={{
                  ...
                  color: "#fff", fontSize: 14, fontWeight: 800, cursor: "pointer", fontFamily: font,
                }}>
            ...
            {/* ═══ REBOUNDS ═══ */}
            <Zone title="Rebounds" icon="🔄" open={openZones.rebounds} onToggle={() => toggleZone("rebounds")}
              badge={(hd.rbControlled + hd.rbDangerous) > 0 ? `${hd.rbControlled + hd.rbDangerous}` : null} accentColor={t.cyan}>
            ...
            {/* ═══ DISTRIBUTION ═══ */}
            <Zone title="Distribution" icon="📤" open={openZones.distribution} onToggle={() => toggleZone("distribution")}
              badge={distTotal > 0 ? `${distTotal}` : null} accentColor={t.gold}>
              ...
              {distTotal > 0 && (
                <div style={{ marginTop: 8, padding: "6px 10px", background: t.bg, borderRadius: 6, fontSize: 11, color: t.dim, display: "flex", justifyContent: "space-between" }}>
            ...
            {/* ═══ SWEEPER-KEEPER ═══ */}
            <Zone title="Sweeper-Keeper" icon="🏃" open={openZones.sweeper} onToggle={() => toggleZone("sweeper")}
              badge={sweeperTotal > 0 ? `${sweeperTotal}` : null} accentColor={t.teal}>
            ...
            {/* ═══ NOTES ═══ */}
            <Zone title={`${half} Notes`} icon="🗣️" open={openZones.notes} onToggle={() => toggleZone("notes")} accentColor={t.gold}>
              <textarea value={hd.note} onChange={e => setH("note", e.target.value)}
                placeholder="Tap to type or use voice-to-text…"
                style={{ width: "100%", minHeight: 60, padding: 10, borderRadius: 8, border: `1px solid ${t.border}`, background: t.bg, color: t.text, fontSize: 13, fontFamily: font, resize: "vertical", outline: "none", boxSizing: "border-box" }} />
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 4 }}>
                {SUB_REASONS.map(r => (
                  <button key={r} onClick={() => handleSubstitute(r)} style={{
                    padding: "10px 6px", borderRadius: 8, border: `1px solid ${t.orange}33`,
                    background: t.orange + "08", color: t.orange, fontSize: 10, fontWeight: 600,
                    cursor: "pointer", lineHeight: 1.3, textAlign: "center", fontFamily: font,
                  }}>{r}</button>
                ))}
              </div>
            </div>
            ...
            <div style={{ background: t.card, borderRadius: 12, padding: 16, border: `1px solid ${t.border}`, marginTop: 8 }}>
              ...
              <button onClick={handleEndGame} disabled={sessionType === "match" && !result} style={{
                ...
                fontSize: 15, fontWeight: 700, cursor: (sessionType === "training" || result) ? "pointer" : "not-allowed", fontFamily: font,
              }}>💾 Save {sessionType === "training" ? "Session" : "Match"}</button>
            </div>
          </>
        )}
      ...
      {showSetup && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.88)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
          <div style={{ background: t.card, borderRadius: 16, width: "100%", maxWidth: 380, padding: 20 }}>
            ...
            <button onClick={() => setShowSetup(false)} style={{ width: "100%", marginTop: 16, padding: 12, borderRadius: 10, background: club?.primary_color || t.accent, color: "#fff", border: "none", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: font }}>Close</button>
          </div>
        </div>
      )}
    </div>

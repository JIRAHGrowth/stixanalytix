...
export function aggregateMatches(matches, opts = {}) {
  ...
  const { shotEvents, distEvents, sweeperEvents, oneVOneEvents, goalsConceded, crossEvents } = opts;
  const eventDriven = shotEvents !== undefined || distEvents !== undefined ||
                      sweeperEvents !== undefined || oneVOneEvents !== undefined ||
                      goalsConceded !== undefined || crossEvents !== undefined;
  ...
  // ── Crosses ──
  // cross_events (added 2026-07-14) stores per-cross rows with keeper_id,
  // so multi-keeper matches now split correctly. Fall back to match-column
  // sums for legacy matches without cross_events rows.
  const crosses = eventDriven && crossEvents !== undefined
    ? bucketCrosses(crossEvents || [])
    : {
...
// Bucket cross_events into the same shape the Crosses tab already renders.
// gk_action enum (per cross_events schema): catch | punch | tip_over |
// stayed_on_line | missed | defender_cleared. Mapping to the 3-bucket UI:
//   claimed = catch OR tip_over
//   punched = punch
//   missed  = missed (or missed/misjudged historical variant)
//   total   = every cross row (including stayed_on_line + defender_cleared —
//             they still represent crosses that came into our box)
function bucketCrosses(crossEvents) {
  const b = { claimed: 0, punched: 0, missed: 0, total: 0 };
  crossEvents.forEach(e => {
    b.total++;
    const a = String(e.gk_action || '').toLowerCase();
    if (a === 'catch' || a === 'tip_over' || a === 'tip-over') b.claimed++;
    else if (a === 'punch') b.punched++;
    else if (a === 'missed' || a === 'missed/misjudged') b.missed++;
  });
  return b;
}

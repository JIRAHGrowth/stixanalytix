...
export async function fetchAnalyticsBundle(supabase, coachId) {
  const [matchRes, goalRes, attrRes, shotRes, distRes, sweeperRes, ovoRes] = await Promise.all([
    ...
    // Distribution + sweeper + 1v1 events — same opp-exclusion pattern for
    // dist. sweeper_events has no keeper_team column (all rows are our GKs).

...
export async function fetchAnalyticsBundle(supabase, coachId) {
  const [matchRes, goalRes, attrRes, shotRes, distRes, sweeperRes, ovoRes, crossRes] = await Promise.all([
    ...
    // Exclude opposition-keeper events from analytics. Pre-feature rows
    // (keeper_team IS NULL) are treated as "us" — see migration
    // 20260606_keeper_team_attribution.sql.
    supabase.from("shot_events").select("*").eq("coach_id", coachId)
      .or("keeper_team.is.null,keeper_team.neq.opp"),
    // Distribution + sweeper + 1v1 + cross events — same opp-exclusion pattern.
    // sweeper_events + one_v_one_events have no keeper_team column (all rows
    // are our GKs). cross_events has keeper_team so we filter it.
    supabase.from("distribution_events").select("*").eq("coach_id", coachId)
      .or("keeper_team.is.null,keeper_team.neq.opp"),
    supabase.from("sweeper_events").select("*").eq("coach_id", coachId),
    supabase.from("one_v_one_events").select("*").eq("coach_id", coachId),
    supabase.from("cross_events").select("*").eq("coach_id", coachId)
      .or("keeper_team.is.null,keeper_team.neq.opp"),
  ]);
  return {
    ...
    distEvents: distRes.data || [],
    sweeperEvents: sweeperRes.data || [],
    oneVOneEvents: ovoRes.data || [],
    crossEvents: crossRes.data || [],
  };
}
 ...
 *
 * Returns { goalsConceded, goalsScored, shotEvents, distEvents,
 *          crossEvents, sweeperEvents, oneVOneEvents, keeper }.
 *
 * Phase B (2026-07-27) — previously returned only goals + saves. Distribution,
 * crosses, sweeper and 1v1 rows now live on the DB and were invisible on the
 * match-detail page before this bundle was widened. Same opposition-keeper
 * exclusion pattern as fetchAnalyticsBundle: keeper_team NULL (pre-feature) is
 * treated as "us"; sweeper + 1v1 have no keeper_team column at all.
 */
export async function fetchMatchDetailBundle(supabase, matchId, keeperId, secondaryKeeperId = null) {
  const timeOrder = { ascending: true, nullsFirst: false };
  const [gcRes, gsRes, seRes, deRes, ceRes, swRes, ovRes, kRes, k2Res] = await Promise.all([
    ...
    supabase.from("goals_scored").select("*")
      .eq("match_id", matchId).order("timestamp_seconds", timeOrder),
    // Exclude opposition-keeper rows from the analyzed keeper's match view.
    // NULL keeper_team rows (pre-feature data) are treated as "us".
    supabase.from("shot_events").select("*")
      .eq("match_id", matchId)
      .or("keeper_team.is.null,keeper_team.neq.opp")
      .order("timestamp_seconds", timeOrder),
    supabase.from("distribution_events").select("*")
      .eq("match_id", matchId)
      .or("keeper_team.is.null,keeper_team.neq.opp")
      .order("timestamp_seconds", timeOrder),
    supabase.from("cross_events").select("*")
      .eq("match_id", matchId)
      .or("keeper_team.is.null,keeper_team.neq.opp")
      .order("timestamp_seconds", timeOrder),
    supabase.from("sweeper_events").select("*")
      .eq("match_id", matchId).order("timestamp_seconds", timeOrder),
    supabase.from("one_v_one_events").select("*")
      .eq("match_id", matchId).order("timestamp_seconds", timeOrder),
    keeperId
      ? supabase.from("keepers").select("*").eq("id", keeperId).maybeSingle()
      : Promise.resolve({ data: null }),
    secondaryKeeperId
      ? supabase.from("keepers").select("*").eq("id", secondaryKeeperId).maybeSingle()
      : Promise.resolve({ data: null }),
  ]);
  return {
    ...
    distEvents: deRes.data || [],
    crossEvents: ceRes.data || [],
    sweeperEvents: swRes.data || [],
    oneVOneEvents: ovRes.data || [],
    keeper: kRes.data,
    secondaryKeeper: k2Res.data,
  };
}
